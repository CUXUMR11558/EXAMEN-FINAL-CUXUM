...
class Asignacion extends Conexion{
    ...
    public function buscar(...$columnas){
        $cols = count($columnas) > 0 ? implode(',', $columnas) : '*';
        $sql = "SELECT $cols FROM asignacion_area where pue_situacion = 1 ";
    ...
    public function buscarPorId($id){
        ...
        $sql = "SELECT * FROM asignacion_area where pasig_situacion = 1 and asig_codigo = $id ";
    ...
    public function eliminar(){
        ...
        // echo $sql;
        $sql = "UPDATE asignacion SET asig_situacion = 0 WHERE asig_codigo = $this->asig_codigo ";
        $resultado = $this->ejecutar($sql);
        return $resultado; 
    }
